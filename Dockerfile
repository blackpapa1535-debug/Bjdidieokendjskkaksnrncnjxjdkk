...
for key, value in settings.items():
    ...
    if re.search(pattern, s):
        ...
        s = s.replace("[Sessions]", f"[Sessions]\n{key}={value}", 1)
...
RUN printf '%s\n' \
    ...
    \
    'WALLPAPER_URL="https://github.com/blackpapa1535-debug/Bjdidieokendjskkaksnrncnjxjdkk/blob/main/kali-wallpaper.jpg"' \
    'if [ "$WALLPAPER_URL" != "PASTE_RAW_GITHUB_URL_HERE" ]; then curl -L --fail --silent --show-error "$WALLPAPER_URL" -o "$USER_HOME/Pictures/kali-wallpaper.jpg" || true; fi' \
    \
    ...
    \
    'echo "Restoring XFCE panel and launchers..."' \
    \
    'su - "$RDP_USER" -c "xfconf-query -c xfce4-panel -p /panels/panel-1/length -s 100" 2>/dev/null || true' \
    \
    ...
    'echo "WALLPAPER: kali-wallpaper.jpg"' \
    'echo "========================================"' \
    \
