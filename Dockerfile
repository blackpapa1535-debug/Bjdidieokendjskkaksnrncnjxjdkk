...
RUN mkdir -p /opt/telegram && \
    ...
    wget -q --show-progress \
        ...
        -O telegram.tar.xz && \
    tar -xJf telegram.tar.xz -C /opt/telegram --strip-components=1 && \
...
RUN printf '%s\n' \
    ...
    'su - "$RDP_USER" -c "xfconf-query -c xfce4-notifyd -p /notify-on -s false" 2>/dev/null || true' \
    \
    'cat > "$USER_HOME/.config/autostart/disable-notifications.desktop" <<EOF' \
    '[Desktop Entry]' \
    'Type=Application' \
    'Name=Disable Desktop Notifications' \
    'Exec=sh -c "xfconf-query -c xfce4-notifyd -p /do-not-disturb -s true 2>/dev/null || true"' \
    'OnlyShowIn=XFCE;' \
